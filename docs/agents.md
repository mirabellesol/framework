...
## Implementation in Mirabelle Agent Framework
...
Our Mirabelle Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new MirabelleAgent({
  llm: new OllamaChatLLM(), // for more explore 'Mirabelle-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'Mirabelle-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'Mirabelle-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/Mirabelle_advanced.ts) to see how to modify them.
