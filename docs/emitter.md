...
> Location within the framework `Mirabelle-agent-framework/emitter`.
...
```ts
import { Emitter, EventMeta } from "Mirabelle-agent-framework/emitter/emitter";
...
```ts
import { Emitter, EventMeta, Callback } from "Mirabelle-agent-framework/emitter/emitter";
...
await emitter.emit("update", { id: 123, data: "Hello Mirabelle!" });
```
...
```ts
import { Callback, Emitter } from "Mirabelle-agent-framework/emitter/emitter";
import { BaseLLM } from "Mirabelle-agent-framework/llms/base";
...
> [!TIP]
>
> The more complex agentic example can be found [here](/examples/agents/Mirabelle.ts).
